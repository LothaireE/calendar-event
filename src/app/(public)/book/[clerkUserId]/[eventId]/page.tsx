...
export default async function BookEventPage (props: { params: Promise<{ clerkUserId: string, eventId: string }> }) {
    ...
    const fullName = displayFullName(calendarUser.firstName, calendarUser.lastName)
    ...
    const validTimes = await getValidTimesFromSchedule(
        eachMinuteOfInterval({start: startDate, end: endDate}, {step: minutesGap}), event
    ) // passing an array of every fifteen minutes value between the start date and 2 month later
    
    if (validTimes.length === 0) {
        return <NoTimeSlots event={event} calendarUser={{...calendarUser, fullName}} />
    }
...
export function NoTimeSlots ({
    ...
    calendarUser : { id: string, firstName : string | null, lastName: string | null , fullName: string }
}) {
