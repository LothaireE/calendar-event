import { uuid, text, integer, pgTable, varchar, boolean, timestamp, index, pgEnum } from "drizzle-orm/pg-core";
...
export const EventsTable = pgTable("events", { //pg postgres is case-sensitive, so use camelCase for table names
...
export const ScheduleAvailabilityTable = pgTable("scheduleAvailabilities", {
  id: uuid("id").primaryKey().defaultRandom(),
  scheduleId: uuid("scheduleId").notNull().references(() => ScheduleTable.id, { onDelete: "cascade"}), // Foreign key to schedules table
